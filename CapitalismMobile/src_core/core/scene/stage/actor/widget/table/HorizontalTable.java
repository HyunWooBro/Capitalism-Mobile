...
/**
 * 가로로만 TableCell을 배치할 수 있는 특수 Table이다. 기존 Table의 경우 중간에 TableCell을 추가하거나 
 * 제거하기 위해서는 직접 Table을 정리한 후 다시 만들거나 아예 새로 Table을 생성해야 하지만 
 * HorizontalTable은 어디든 추가 및 삭제를 지원하고 자동으로 Table을 재생성한다.</p>
 * 
 * 삽입되는 TableCell의 가로 또는 세로 span은 1만 허용되며 수정하더라도 무시된다.</p>
 * 
 * @author 김현우
 */
public class HorizontalTable extends Table<HorizontalTable> {
