...
public class SoundManager {
	...
	private android.media.AudioManager m_AudioManager;// 사운드를 관리할 오디오 매니저
	private Context m_Activity;// 애플리케이션의 컨텍스트
	 
	public void  Init(Context _context){
		m_SoundPool=new SoundPool(4, android.media.AudioManager.STREAM_MUSIC, 0);// 사운드풀 생성
		m_SoundPoolMap=new HashMap();
		m_AudioManager=(android.media.AudioManager)_context.getSystemService(Context.AUDIO_SERVICE);// 핸들 값
	...
	public void play(int _index){
		// 사운드 재생
		float streamVolume=m_AudioManager.getStreamVolume(android.media.AudioManager.STREAM_MUSIC);
		streamVolume=streamVolume/m_AudioManager.getStreamMaxVolume(android.media.AudioManager.STREAM_MUSIC);
